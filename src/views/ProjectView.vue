        ...
        <div v-for="(project, index) in projects" :key="project.id">
            <a :href="project.html_url" target="_bl">
                <div class="bg-blue-300 mb-4 p-2 rounded-md shadow-md">
                    ...
                    <a :href="project.homepage" target="_blank" class="text-sm text-blue-600">{{project.homepage}}</a>
                </div>
            </a>
        </div>
    </div>
...
onMounted(async () => {
    try {
        loading.value = true;
        const res = await fetch('https://api.github.com/users/achmadzackyr/repos');
